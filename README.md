...
First, demographic information include age, sex, and BMI. We do have both genders and we have childs and adults among patients. Such age, gender and BMI variations impact a lot on the time duration of each respiratory cycle. 
Second, the location on the body for data collection, impact on the frequency profile of each sample. Our data is collected from seven different locations on the body; Trachea, Anterior left/ right, Posterior left/right, and lateral left/right. Based on our knowledge lateral locations will have lower frequency ranges comparing trachea for the same class of abnormal sounds. 
Third, recording equipment is another key factor which influences signal to noise ratio tremendously (SNR). Our data is collected by  four different equipments, while there is a huge gap among their performance. For instance, the SNR for signals collected by “3M Litmmann 3200 Electronic Stethoscope (Litt3200)” is much higher compared with the other devices. 
...
Fifth, considering the number of parameters to be trained (over 16 million) in the CNN implementation, the dataset size is very small (4827 total samples) and this restricts the learning capability of the network. 
