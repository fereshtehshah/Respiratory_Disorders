...
## Dataset Evaluation
Several factors have a huge impact on data cleaning and classification accuracy in our work. Those factors are 1) demographic information, 2) devices used for collecting sound data, 3)location on the body for which data is collected and 4) unbalanced number of samples in each class as well as different time duration for each recorded sample. Since data is not categorized considering all these factors, preprocessing, feature extraction, and overall data cleaning part was so complicated. Hence, it hugely impacted on classification accuracy. <br>
We elaborate more on those factors here:<br>
First, demographic information include age, sex, and BMI. We do have both genders and we have childs and adults among patients. Such age, gender and BMI variations impact a lot on the time duration of each respiratory cycle. <br>
Second, the location on the body for data collection, impact on the frequency profile of each sample. Our data is collected from seven different locations on the body; Trachea, Anterior left/ right, Posterior left/right, and lateral left/right. Based on our knowledge lateral locations will have lower frequency ranges comparing trachea for the same class of abnormal sounds. <br>
Third, recording equipment is another key factor which influences signal to noise ratio tremendously (SNR). Our data is collected by  four different equipments, while there is a huge gap among their performance. For instance, the SNR for signals collected by “3M Litmmann 3200 Electronic Stethoscope (Litt3200)” is much higher compared with the other devices. <br>
Forth, the format of the data itself impacts on our final accuracy. All the clips were of different lengths, ranging from 0.2 to 16.2 seconds. The clips were also not sampled at the same sampling rate. This required us to augment the data through zero-padding, cropping, filtering, and up-sampling or down-sampling, which removed from the truth of the actual data and could cause problems in the training process.<br>
Fifth, considering the number of parameters to be trained (over 16 million) in the CNN implementation, the dataset size is very small (4827 total samples) and this restricts the learning capability of the network. <br>
To recap, it is crucial for future steps, to revise our models to account for such discussed differences. It is highly important to categorize data with more care regarding those issues, and spend more time and effort on the preprocessing phase, in the future. <br>
